  ...
  >
    <el-card class="header-card">
      <div class="header-content">
        <div class="header-left">
          ...
          <p class="header-subtitle">管理平台所有商家账号</p>
        </div>
    ...
    <el-card class="search-card">
      <el-form :inline="true" :model="queryForm" class="query-form">
        ...
        <el-form-item label="商家类型" class="search-item">
          <el-select
            v-model="queryForm.shopType"
            placeholder="请选择商家类型"
            clearable
            style="width: 150px"
          >
    ...
    <el-card class="table-card">
      <el-table :data="merchantList" border stripe v-loading="loading" fit>
        <el-table-column
          type="index"
          label="序号"
          width="70"
    ...
    <el-dialog
      :title="merchantForm.id ? '🏪 编辑商家信息' : '➕ 添加商家'"
      :visible.sync="dialogVisible"
      width="650px"
      class="merchant-edit-dialog"
    >
      <div class="dialog-content">
        <div class="merchant-info-header" v-if="merchantForm.id">
          <div class="merchant-avatar">
            <img
              v-if="merchantForm.avatar"
              :src="getImageUrl(merchantForm.avatar)"
              alt="头像"
            />
          ...
          <div class="merchant-basic-info">
            <div class="merchant-name">
              {{ merchantForm.shopName || merchantForm.username }}
            </div>
            <div class="merchant-meta">
              <el-tag size="mini" type="warning">
                {{ merchantForm.shopType || "商家" }}
              </el-tag>
              <el-tag
                size="mini"
                :type="merchantForm.status === 1 ? 'success' : 'info'"
              >
                {{ merchantForm.status === 1 ? "启用" : "禁用" }}
              </el-tag>
            </div>
          </div>
        </div>

        <el-divider v-if="merchantForm.id">
        ...
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item label="商家头像" v-if="merchantForm.id">
          ...
          <el-form-item label="商家名称" prop="shopName">
            <el-input
              ...
              placeholder="请输入商家名称"
            >
              <template slot="prepend"><i class="el-icon-s-shop"></i></template>
            </el-input>
          </el-form-item>
          <el-form-item label="商家类型" prop="shopType">
            <el-select
              v-model="merchantForm.shopType"
              placeholder="请选择商家类型"
              style="width: 100%"
            >
              <el-option label="自营" value="自营"></el-option>
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          if (res.code === 200 || res.code === 0) {
            this.merchantForm.avatar = res.url || res.data;
    ...
    loadMerchantList() {
      ...
      const params = {
        ...
        shopType: this.queryForm.shopType,
      ...
      getUserList(params)
        .then((res) => {
          if (res.code === 200) {
            this.merchantList = res.data.records || res.data;
    ...
    submitForm() {
      this.$refs.merchantFormRef.validate((valid) => {
        if (valid) {
          ...
          if (this.merchantForm.id) {
            updateUser(this.merchantForm, currentRole)
              .then((res) => {
                if (res.code === 200) {
            ...
            addUser(this.merchantForm, currentRole)
              .then((res) => {
                if (res.code === 200) {
